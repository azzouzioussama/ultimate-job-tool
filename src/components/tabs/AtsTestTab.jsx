      ...
      <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          <h2 className="text-lg font-semibold flex items-center gap-2">
            <Activity size={20} className="text-indigo-600" /> {t('ats.title', 'Testeur ATS')}
          </h2>
          <p className="text-xs text-slate-500 mt-1">
            {t('ats.subtitle', "Évaluez la compatibilité entre votre CV et l'offre d'emploi.")}
          </p>
        </div>
        <button
          onClick={onRunATS}
        ...
        >
          ...
          {t('ats.run', "Lancer l'Analyse ATS")}
        </button>
      </div>
      ...
      <div className="flex-grow p-6 bg-slate-50">
        ...
        {isAtsLoading ? (
          <div className="flex flex-col items-center justify-center py-12 text-indigo-600 space-y-3">
            ...
            <p className="text-sm font-medium animate-pulse">{t('ats.analyzing', "L'IA analyse les mots-clés...")}</p>
          </div>
          ...
          <div className="max-w-3xl mx-auto space-y-6">
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 flex flex-col sm:flex-row items-center sm:items-start gap-6 shadow-sm">
              ...
              <div className="text-center sm:text-left mt-2 sm:mt-0">
                <h3 className="text-lg font-bold text-slate-800">{t('ats.scoreTitle', 'Score de compatibilité')}</h3>
                <p className="text-sm text-slate-500 mt-1">
                  {t('ats.scoreDesc', 'Un score supérieur à 80% maximise vos chances de passer les filtres automatiques (ATS) des recruteurs.')}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                <h3 className="font-bold text-slate-800 flex items-center gap-2 mb-4">
                  <FileText size={16} className="text-red-500"/> {t('ats.missingTitle', 'Mots-clés manquants')}
                </h3>
                {atsResult.missingKeywords?.length > 0 ? (
                  ...
                  <p className="text-sm text-green-600 font-medium">
                    {t('ats.missingGood', "Excellent ! Votre CV contient tous les mots-clés essentiels demandés dans l'offre.")}
                  </p>
                )}
              </div>
              ...
              <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                <h3 className="font-bold text-slate-800 flex items-center gap-2 mb-4">
                  <Sparkles size={16} className="text-indigo-500"/> {t('ats.adviceTitle', "Conseils d'amélioration")}
                </h3>
        ...
        /* Empty State */
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-slate-500 max-w-sm">
              {!jobDescription
                ? t('ats.missingJob', "Il vous manque une offre d'emploi. Allez dans l'onglet 'Offre'.")
                : (!cvGenerated && !cvOriginal)
                  ? t('ats.missingCv', "Il vous manque un CV. Allez dans l'onglet 'Mon CV'.")
                  : t('ats.clickToRun', "Cliquez sur 'Lancer l'Analyse ATS' pour évaluer votre CV par rapport à l'offre.")}
            </p>
          </div>
        )}
      </div>
